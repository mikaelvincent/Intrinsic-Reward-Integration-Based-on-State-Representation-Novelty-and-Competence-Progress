...
\end{tabular}
\caption{
Solved-threshold reliability and speed.
Each cell reports the number of seeds that reached the task's solved-return threshold by the end of training and the median training steps (in millions) to first reach that threshold, computed by linearly interpolating between deterministic evaluation checkpoints (Figure~\ref{fig:steps_to_threshold_baselines}).
Medians are taken over successful seeds only.
The ``most reliable baseline'' is selected among Vanilla PPO, ICM, RND, RIDE, and RIAC by highest solve count (ties broken by lower median steps).
Higher solve counts and lower median steps indicate more reliable and faster learning.
}
