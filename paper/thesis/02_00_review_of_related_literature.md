...
Across these themes, prior studies consistently showed that intrinsic rewards can improve exploration when extrinsic rewards are sparse or delayed [1], [2], [3], [4]. However, they also identified a recurring risk that exploration may be drawn toward transitions that remain surprising without improving task learning, especially in stochastic or poorly learnable regions [11], [14]. This limitation motivated approaches that track not only surprise, but also whether model error decreases over time, which is the central idea behind learning progress [5], [13].
