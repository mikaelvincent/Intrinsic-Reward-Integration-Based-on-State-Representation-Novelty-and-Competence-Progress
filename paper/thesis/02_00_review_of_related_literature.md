...
Across these themes, prior studies consistently showed that intrinsic rewards can improve exploration when extrinsic rewards are sparse or delayed {{CIT:3,11,12,20}}. However, they also identified a recurring risk that exploration may be drawn toward transitions that remain surprising without improving task learning, especially in stochastic or poorly learnable regions {{CIT:8,15}}. This limitation motivated approaches that track not only surprise, but also whether model error decreases over time, which is the central idea behind learning progress {{CIT:2,17}}.
