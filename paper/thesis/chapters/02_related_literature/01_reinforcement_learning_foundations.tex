...
The standard formulation is an episodic Markov decision process (MDP), defined by an observation or state space $\mathcal{S}$, an action space $\mathcal{A}$, a transition distribution $P(s_{t+1}\mid s_t,a_t)$, a reward function $r(s_t,a_t,s_{t+1})$, a discount factor $\gamma\in[0,1)$, and an episode horizon $T$.
...
Potential-based reward shaping can preserve optimal policies when it follows a restricted form, but arbitrary shaping terms may change the task objective  \parencite{ng-1999}.
