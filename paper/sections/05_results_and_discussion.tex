...
Across the suite, GLPE (no gate) is the most consistent default: it matches the strongest baseline within uncertainty on \textsc{Humanoid-v5} and remains competitive with the best-performing baseline on the remaining environments (Table~\ref{tab:final_returns_glpe_vs_baselines}).
The full gated variant is best viewed as an optional guardrail in settings where intrinsic shaping can be unproductive or destabilizing.
This is clearest on the sparse-reward \textsc{MountainCar-v0} task, where gating yields the best mean final return and the strongest step-AUC while still solving in all seeds (Tables~\ref{tab:final_returns_glpe_vs_baselines}, \ref{tab:auc_steps_glpe_vs_baselines}, and~\ref{tab:solved_threshold_stats}).
...
Some diagnostic and efficiency plots additionally include auxiliary GLPE variants, including GLPE (cached medians) and single-component ablations, to interpret compute and signal contributions; we treat these as supporting analyses rather than primary baselines.

\input{figures/eval_curves_baselines}
\input{tables/final_returns_glpe_vs_baselines}
\input{tables/auc_steps_glpe_vs_baselines}

Table~\ref{tab:auc_steps_glpe_vs_baselines} summarizes sample efficiency using step-AUC over environment steps, rewarding methods that learn quickly and sustain high returns over the fixed step budget.
...
On \textsc{Ant-v5} and \textsc{HalfCheetah-v5}, the GLPE variants fall within a small gap of the strongest baseline step-AUC.
On \textsc{Humanoid-v5}, uncertainty is large and the ordering between top methods is not stable; GLPE (no gate) overlaps the strongest baseline under both final-return and step-AUC summaries (Tables~\ref{tab:final_returns_glpe_vs_baselines} and~\ref{tab:auc_steps_glpe_vs_baselines}).
The strongest baseline differs across environments and between these summaries (e.g., RIAC leads \textsc{Ant-v5} at the final checkpoint, while RND has the highest step-AUC), underscoring the value of a method that remains consistently competitive across views.
...
Consistent with the gate map in Figure~\ref{fig:glpe_gate_map}, gating rarely disables intrinsic shaping on this task, so the two variants behave almost identically.
...
Notably, RIAC is the most reliable baseline for crossing the full $300$ cutoff (8/8 solved; Table~\ref{tab:solved_threshold_stats}), whereas GLPE (no gate) often stalls slightly below the threshold and therefore yields fewer full-threshold successes despite matching baselines at the 50\% level (Table~\ref{tab:half_solved_threshold_stats}).
...
Across environments, the gate switches off only in localized regions of the visited state distribution, supporting the interpretation of gating as a targeted guardrail: it can prevent unhelpful shaping in sparse-reward settings without broadly suppressing exploration when dense reward already provides strong guidance.
...
We use fixed solved thresholds for these summaries (\textsc{MountainCar-v0}: $-110$, \textsc{BipedalWalker-v3}: $300$, \textsc{Ant-v5}: $5{,}000$, \textsc{HalfCheetah-v5}: $6{,}000$, \textsc{Humanoid-v5}: $3{,}000$), shown as dashed lines in Figure~\ref{fig:eval_curves_baselines}.
...
Comparing the 50\% and 100\% summaries shows that the remaining failures are concentrated in final-threshold crossing on high-variance tasks: on \textsc{Humanoid-v5}, and on \textsc{BipedalWalker-v3} for most methods, many runs reach competent behavior but fewer cross the full solved cutoff (Table~\ref{tab:solved_threshold_stats}).
...
Across \textsc{BipedalWalker-v3}, \textsc{HalfCheetah-v5}, \textsc{Ant-v5}, and \textsc{Humanoid-v5}, GLPE (no gate) remains close to the strongest baseline in wall-clock AUC (Figure~\ref{fig:eval_auc_time}).
...
\textsc{MountainCar-v0} is more sensitive because environment stepping is comparatively cheap, so intrinsic overhead can materially affect wall-clock rankings; in this regime the wall-clock AUC differences among the top methods are smaller than in the step-normalized view.
...
Because caching uses slightly stale medians and can therefore change gating decisions, we treat it as an optional engineering knob rather than part of the core GLPE comparison; empirically, the resulting ``GLPE (cached medians)'' variant can underperform uncached GLPE on exploration-sensitive tasks, most notably \textsc{Humanoid-v5} (Figure~\ref{fig:eval_auc_time}).
Taken together, these results indicate that GLPE (no gate) remains competitive under both step-normalized and wall-clock-normalized views, while gating is most beneficial on sparse-reward \textsc{MountainCar-v0} and can be counterproductive in high-variance regimes such as \textsc{Humanoid-v5}.
