...
def _score_to_beat(
    ...
    if thr is None or not np.isfinite(float(thr)):
        raise ValueError(f"Missing fixed reward threshold for env: {env_id}")
    return float(thr)
...
def plot_steps_to_beat_by_env(
    ...
    *,
    ...
    thresholds_full: dict[str, float] = {str(env_id): float(_score_to_beat(str(env_id))) for env_id in envs}
